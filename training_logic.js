...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const copySparqlBtn = document.getElementById('copy-sparql-btn');
   ...
   document.getElementById('btn-download-offline').addEventListener('click', async () => {
    ...
    // --- NUOVA LOGICA: CARICAMENTO FILE ---
    if(copySparqlBtn) {
        copySparqlBtn.addEventListener('click', async (e) => {
            const queryText = document.getElementById('sparql-query').value;
            try {
                await navigator.clipboard.writeText(queryText.trim());
                const originalText = copySparqlBtn.textContent;
                copySparqlBtn.textContent = 'Copiata!';
                copySparqlBtn.style.backgroundColor = '#d4edda';
                copySparqlBtn.style.color = '#155724';
                
                setTimeout(() => {
                    copySparqlBtn.textContent = originalText;
                    copySparqlBtn.style.backgroundColor = '#0055A5';
                    copySparqlBtn.style.color = 'white';
                }, 2000);
            } catch (err) {
                console.error('Errore Clipboard API: ', err);
                alert('Impossibile copiare. Effettuare la copia manuale.');
            }
        });
    }
    ...
    if(uploadDataForm) {
        uploadDataForm.addEventListener('submit', async (e) => {
            ...
            const formData = new FormData(uploadDataForm);
            ...
            try {
                ...
                statusDiv.textContent = 'Elaborazione lato server in corso...';
                statusDiv.style.color = '#856404';

                // POST all'endpoint Flask
                ...
                // Generazione download automatico del nuovo JSON di cache (Memoria gestita via Blob)
                const blob = await response.blob();
                const downloadUrl = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = downloadUrl;
                a.download = 'data_cache_updated.json';
                document.body.appendChild(a);
                a.click();
                a.remove();
                window.URL.revokeObjectURL(downloadUrl);

                statusDiv.textContent = 'Cache elaborata e scaricata con successo!';
                statusDiv.style.color = '#155724';
                uploadDataForm.reset();

            } catch (error) {
                statusDiv.textContent = `Errore di sistema: ${error.message}`;
